  ...
  useEffect(() => {
    ...
    fetch(`/api/blueprint/${sectorSlug}`)
      .then(async res => {
        ...
        if (!res.ok) throw new Error('Failed to load blueprint data')
        return res.json()
      })
      .then(json => {
        if (json) {
  ...
  if (error || !data) {
    return (
      <div className="w-full max-w-5xl mx-auto flex flex-col items-center justify-center py-24 gap-4">
        <p className="font-syne font-black text-zinc-900 dark:text-white text-2xl">Failed to load blueprint data</p>
        <p className="font-dm-sans text-sm text-zinc-500">{error ?? 'Data not found'}</p>
        <button
          onClick={() => window.location.reload()}
          className="font-dm-mono text-xs border border-zinc-300 dark:border-zinc-700 text-zinc-500 dark:text-zinc-400 px-4 py-2 rounded-lg hover:border-zinc-400 dark:hover:border-zinc-500 transition-colors"
        >
          Retry
        </button>
      </div>
    ...
    <div className="w-full max-w-5xl mx-auto">
      ...
      <div className="flex items-center gap-2 mb-8">
        <Link href="/dashboard/blueprint"
          className="text-[11px] font-medium text-zinc-400 hover:text-zinc-700 dark:hover:text-zinc-300 transition-colors">
          StackMap
        </Link>
        <span className="text-zinc-300 dark:text-zinc-700 text-[11px]">/</span>
        <span className="text-[11px] font-semibold text-zinc-700 dark:text-zinc-300">{data.sector.label}</span>
      </div>
      ...
      <div className="flex flex-col lg:flex-row lg:items-start gap-8 mb-10">
        <div className="flex-1">
          <div className="flex items-center gap-3 mb-3">
            <h1 className="font-syne font-black text-4xl text-zinc-900 dark:text-white tracking-tighter">
              {data.sector.label}
            </h1>
          </div>
          ...
          <div className="flex items-center gap-4">
            <span className="font-dm-mono text-[10px] text-zinc-600 uppercase tracking-widest">
              {data.tasks.length} {data.tasks.length === 1 ? 'task' : 'tasks'} · {categories.length} {categories.length === 1 ? 'category' : 'categories'}
            </span>
            {selectedIds.size > 0 && (
      ...
      {/* Upgrade prompt */}
      {showUpgradePrompt && (
        <div className="mb-6 flex items-center justify-between bg-zinc-50 dark:bg-zinc-900 border border-[#CAFF32]/20 rounded-xl px-5 py-4">
          <div>
            <p className="font-syne font-bold text-sm text-zinc-900 dark:text-white mb-0.5">Free plan limit reached</p>
            <p className="font-dm-sans text-xs text-zinc-500">Upgrade to Pro to add more tools to your stack</p>
          </div>
          ...
          <div key={category}>
            ...
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="font-dm-mono text-[10px] text-zinc-700">
                {grouped[category].length} {grouped[category].length === 1 ? 'task' : 'tasks'}
              </span>
            </div>
      ...
      {selectedIds.size > 0 && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-50">
          <div className="flex items-center gap-4 bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-700 rounded-2xl px-6 py-4 shadow-xl">
            <div>
              <p className="font-syne font-black text-sm text-zinc-900 dark:text-white">
                {selectedIds.size} {selectedIds.size === 1 ? 'task' : 'tasks'} selected
              </p>
              <p className="font-dm-mono text-[10px] text-zinc-500">
                {coverage.covered} {coverage.covered === 1 ? 'tool' : 'tools'} available
              </p>
            </div>
            <button
