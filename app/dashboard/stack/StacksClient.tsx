...
export default function StacksClient({ initialStacks }: { initialStacks: Stack[] }) {
  ...
  if (stacks.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-zinc-300 dark:border-zinc-800 p-16 text-center">
        <p className="font-semibold text-zinc-900 dark:text-white text-lg mb-2">Aucun stack pour l'instant</p>
        <p className="text-zinc-500 text-sm mb-6">Décris ton objectif pour générer ton premier stack IA.</p>
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <AnimatePresence>
        {stacks.map((stack) => {
          const date = new Date(stack.created_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric' })
          ...
          return (
            ...
            >
              ...
              <div className="flex items-center gap-5 py-3 border-t border-b border-zinc-100 dark:border-zinc-800/60 mb-3">
                {[
                  { label: 'Coût', value: `${stack.total_cost}€/mois` },
                  { label: 'ROI', value: `+${stack.roi_estimate}%`, accent: true },
                  { label: 'Score', value: `${stack.score}/100` },
                  { label: 'Agents', value: String(stack.agent_ids?.length ?? 0) },
                ].map((m, i) => (
                  <div key={i}>
