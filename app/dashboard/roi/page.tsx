...
export default async function ROIPage() {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto">
      <div className="mb-10">
        <h1 className="font-syne font-black text-4xl text-zinc-900 dark:text-white tracking-tighter mb-2">ROI Overview</h1>
        <p className="text-zinc-500 text-sm">Track your return on investment across all your AI stacks</p>
      </div>
      ...
      {stacks.length === 0 ? (
        <div className="border border-dashed border-zinc-300 dark:border-zinc-800 p-16 text-center rounded-xl">
          ...
          <p className="text-zinc-500 text-sm mb-8">Create your first stack to start tracking ROI</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          {[
            { label: 'Total Saved', value: `+${Math.round(totalROI / stacks.length)}%`, color: 'text-[#CAFF32]' },
            { label: 'Per Month',   value: `${totalCost}€`,                              color: 'text-zinc-900 dark:text-white' },
            { label: 'Avg Score',    value: `${avgScore}/100`,                            color: 'text-[#38bdf8]' },
          ].map((m, i) => (
      ...
      <div className="bg-zinc-50 dark:bg-zinc-900/30 border border-dashed border-zinc-200 dark:border-zinc-800/50 p-8 text-center rounded-xl">
        <p className="font-dm-mono text-[10px] text-zinc-400 dark:text-zinc-600 uppercase tracking-[0.2em] mb-2">Pro Feature</p>
        <p className="text-zinc-500 dark:text-zinc-400 text-sm">Progress charts, before/after AI comparison, and 12-month projections available with Pro plan.</p>
      </div>
    </div>
  )
}
